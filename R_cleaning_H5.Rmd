...
DF <- read.csv("raw_data_H5_all.csv")
summary(DF)
...
DF_clean <- DF %>%
  ...
  summarise(avg_x = mean(`Acceleration x (m/s^2)`),avg_y = mean(`Acceleration y (m/s^2)`), avg_z = mean(`Acceleration z (m/s^2)`),abs_acceleration = mean(`Absolute acceleration (m/s^2)`))
```
## Exporting the data to CSv
```{r}
write.csv(DF_clean, "Clean_data_H5.csv")
```
